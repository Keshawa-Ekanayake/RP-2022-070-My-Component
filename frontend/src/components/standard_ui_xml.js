...
export default class standard_UI_xml extends Component {
    ...
    generateCode(response) {
        ...
        if (response[5].toFixed(2) == TextBoxValue) {
            var textboxString = '"' + this.state.textbox + '"';
            String.prototype.remove = function (s) { return this.replace(s, '') }
            if (textboxString.includes(",")) {
                var textboxarray = textboxString.split(',');
                console.log('Test Array', textboxarray[1].remove('"'));
                for (let i = 0; i < textboxarray.length; i++) {
                    code = code + UIGeneratorPage.generateCodeFortextbox(textboxarray[i].remove('"'));
                    genXMLCode = genXMLCode + UIGeneratorPage.generateXMLTextBoxTag(textboxarray[i].remove('"'), textboxarray[i].remove('"'), 'string', 'Placeholder', this.state.textboxallign, 'Right', this.state.textboxmargin);
                                        ...
                                        <div class="card-body">
                                            <center>
                                                <form onSubmit={this.onSubmit}>
                                                    ...
                                                    <button type="submit" className="btn btn-primary" id="submitBtn" style={{ width: "200px" }} onClick={this.handlegeneratebtn}>Generate</button><br /><br />
